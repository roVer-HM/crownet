...
void AppRxInfo::calcAvgPacketSize(Packet *packetIn){
    ...
    if (burstTag == nullptr || burstTag->getBurstPktCount()==1){
        ...
        //Just use datalengt.
        // x_1 = a*y + (1-a)*x_0 ==> x_0 + a*y - x_0*a ==> x_0 + (y-x_0)*a
        avg_packet_size = avg_packet_size + (b(packetIn->getDataLength()).get() - avg_packet_size)*ema_smoothing_packet_size;
        EV_DETAIL << "No burst track packet length. packetIn: " << packetIn->getDataLength() << " avg: "
                << avg_packet_size << "b " << this->str() << endl;
        ...
        if (getNodeId() <= 0){
            ...
            avg_packet_size = avg_packet_size + (b(packetIn->getDataLength()).get() - avg_packet_size)*ema_smoothing_packet_size;
            EV_DETAIL << "PerAppInfo: Found packet belonging to NEW burst. --> Meter packet and. " << burstTag->str()
                    << " packetIn: " << burstTag->getBurstSize() << " avg: " << avg_packet_size << "b " << this->str() << endl;;
        } else {
            ...
            if (burstIdSet.contains(burstTag->getBurstCreationTime())){
                ...
                EV_DETAIL << "PerSourceInfo: Found packet belonging to previous burst. --> Skip packet. " << burstTag->str()
                        << this->str() << endl;
            } else {
                ...
                avg_packet_size = avg_packet_size + (b(packetIn->getDataLength()).get() - avg_packet_size)*ema_smoothing_packet_size;
                EV_DETAIL << "PerSourceInfo: Found packet belonging to NEW burst. --> Meter packet and save id. "
                        << burstTag->str() << burstTag->str() << " packetIn: " << burstTag->getBurstSize() << " avg: "
                        << avg_packet_size << "b " << this->str() << endl;
            }
        }
    }
