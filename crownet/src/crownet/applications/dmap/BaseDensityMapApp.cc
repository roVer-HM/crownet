...
bool BaseDensityMapApp::mergeReceivedMap(Packet *packet) {
  ...
  if (header->getVersion() == MapType::SPARSE){
      ...
      for (int i = 0; i < numCells; i++) {
          ...
          /**
           *  extract sourceEntryDist from packet. This distance is the distance from which the Entry was generated by the
           *  original 'node'. The sender might be the original node but does not have to be. Further more the
