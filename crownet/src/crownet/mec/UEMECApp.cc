...
void UEMECApp::initialize(int stage)
{
    ...
    selfStart = new cMessage("selfStart");
    selfStop = new cMessage("selfStop");
    selfBeacon = new cMessage("selfBeacon");
    selfEval = new cMessage("selfEval");
    ...
    char *sourceSimbolicAddress = (char*)getParentModule()->getFullName();
    ...
    scheduleAt(simTime() + startTime, selfStart);
    scheduleAt(simTime() + evalPeriod, selfEval);
}

void UEMECApp::handleMessage(cMessage *msg)
{
    if (msg->isSelfMessage() && !std::strcmp(msg->getName(), selfStart->getName()))
    {
        socketD2D.bind(localPortD2D);
        mecAppSendStart();
    } else if (msg->isSelfMessage() && !std::strcmp(msg->getName(), selfEval->getName()))
    {
        evaluate();
        scheduleAt(simTime() + evalPeriod, selfEval);
    } else if (msg->isSelfMessage() && !std::strcmp(msg->getName(), selfBeacon->getName()))
    {
        disseminateBeacon();
        scheduleAt(simTime() + period, selfBeacon);
    } else if (!msg->isSelfMessage() && socketMEC.belongsToSocket(msg))
    {
        inet::Packet* packet = check_and_cast<inet::Packet*>(msg);
        inet::L3Address ipAdd = packet->getTag<L3AddressInd>()->getSrcAddress();

        if (ipAdd == deviceAppAddress || ipAdd == inet::L3Address("127.0.0.1") || ipAdd == inet::L3Address("0.0.0.0"))
        {
            auto deviceAppPacket = packet->peekAtFront<DeviceAppPacket>();
            if(strcmp(deviceAppPacket->getType(), ACK_START_MECAPP) == 0) mecDeviceAppAck(msg);
        }
        else
        {
        ...
        delete msg;
    } else if (!msg->isSelfMessage() && socketD2D.belongsToSocket(msg))
    {
...
void UEMECApp::mecDeviceAppAck(cMessage *msg)
{
    ...
    if(pkt->getResult() == true)
    {
        ...
        scheduleAt(simTime() + 1, selfBeacon);
    }
    else
    {
