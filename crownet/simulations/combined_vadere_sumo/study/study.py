...
def main(base_path):
    ...
    # ------------------------------------------------------------------------------------------------------------
    simtime = UnitValue.s(360.0)

    # parameter variation: define parameters to be varied
    configs = ["mfCombined_Study", "mfSumo_Study"]
    ...
    for config in configs:
        run_parameter_study(base_path, MobilitySimulatorType.SUMO, config, par_var(config))
...
def analyze_parameter_study(base_path: str, config: str, var_parameter: Dict[str, str], vectors: Dict[str, List[str]],
    ...
    suq_run = SuqcRun(base_path, run_prefix=config)
    print(suq_run.get_simulations())
