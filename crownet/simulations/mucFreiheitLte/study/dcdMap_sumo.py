...
def main(base_path):
    ...
    par_var = [
        {
            "omnet": {"*.traci.launcher.sumoCfgBase": "sss"},
            "omnet": {"*.traci.launcher.sumoCfgBase": "aaa"},
        },
    ]
    par_var = OmnetSeedManager(
        par_variations=par_var, rep_count=3, omnet_fixed=False, vadere_fixed=None
    ...
    model = SumoCommand().write_container_log().omnet_tag("latest")
    model = (
        VadereOppCommand()
        .write_container_log()
        .omnet_tag("latest")
