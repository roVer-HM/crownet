...
def get_run_map(study: SuqcRun, rep_count=10):
    ...
    run_map.update(
        { f"ped_{run}_0": dict(rep=r(rep_count), lbl=f"ped_{run}_0") for run in [10, 26, 50, 76, 100, 126, 150, 176, 200] }
    )
    run_map.update(
        { f"415x394_{c}": dict(rep=r(rep_count), lbl=f"415x394_{c}", density=d) for d, _, c in densities }
    )
    run_map.update(
        { f"207x196_{int(c/4)}": dict(rep=r(rep_count), lbl=f"207x196_{int(c/4)}", density=d) for d, _, c in densities }
    )
...
def get_convergence_time(map:pd.DataFrame, time_slice, err) -> dict:
    ...
    df = map.loc[pd.IndexSlice[:, time_slice, ("map_count_mean_n", "map_glb_count_n")],].unstack("data").copy(deep=True)
    ...
    for g, _df in df.groupby(["sim"]):
        ret.append([g, _df["convergence_mask"].idxmax()[-1]-time_slice.start , map.loc[pd.IndexSlice[g, :, "map_glb_count"],].max()])
...
def plot_positions(study: SuqcRun, run_map: dict):
    ...
    for pos_df, kwargs in position_data:
        ...
        for idx, run_id in enumerate(rep_list):
            ...
            ax.plot("x", "y", label="nodes stationary for total simulation", data=pos_df.loc[run_id, :, :, True],  markersize=3, marker='o', linestyle='none', color="red")
            ax.plot("x", "y", label="nodes removed after t=40s", data=pos_df.loc[run_id, :, :, False], markersize=3, marker='o',linestyle='none',  color="blue")
            ax.plot(bound_poly[0], bound_poly[1], color="red")
        fig.legend(*axes[0].get_legend_handles_labels(), loc="lower center", ncol=2, bbox_to_anchor=(.5,0), bbox_transform=fig.transFigure)
