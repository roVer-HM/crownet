...
def main(base_path):
    ...
    par_var = [
        var(ped, opp_seed=s[0], pos_seed=s[1])
        for ped, s in itertools.product([10, 26, 50, 76, 100, 126, 150, 176, 200], seed)
    ]
    par_var.extend(
        [
            var(density, opp_seed=s[0], pos_seed=s[1], cfg_tpl="full_")
            for density, s in itertools.product(range(5), seed)
        ]
    )
    par_var.extend(
        [
            var(density, opp_seed=s[0], pos_seed=s[1], cfg_tpl="quarter_")
            for density, s in itertools.product(range(5), seed)
        ]
    )
