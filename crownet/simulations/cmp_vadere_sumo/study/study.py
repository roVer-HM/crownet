...
from omnetinireader.config_parser import UnitValue
...
from crownetutils.omnetpp.scave import ScaveTool, SqlEmptyResult
from crownetutils.analysis.common import SuqcStudy
from crownetutils.analysis.mobility_model_compare import (
    simulations_from_folder,
...
def main(base_path):
    ...
    def var_parameter(c):
        if "Tcp" in c:
            ...
            return {"module": "World.pNode[0].app[0]", "name": "replyLength"}
        # default:
        return {
            "module": "World.pNode[0].app[0].scheduler",
    ...
    if os.path.exists(qoi_results_path):
        ...
        confirmation = input(
            f'         Really delete "{qoi_results_path}" to re-run analysis? (y/N)'
        )
    ...
    if not os.path.exists(qoi_results_path):
        ...
        for config in configs:
            _analysis_path = os.path.join(get_results_dir(base_path), config)
            ...
            if not os.path.exists(
                _qoi_part_path
            ):  # or not os.path.exists(_qoi_t_part_path):
                # no existing data for this configuration - need to run analysis
                qoi, qoi_t = analyze_parameter_study(
                    base_path,
                    config,
                    var_parameter(config),
                    vectors_analyzed(config),
                ...
                if os.path.exists(_qoi_t_part_path):
                    qoi, qoi_t = (
                        pd.read_csv(
                            _qoi_part_path, sep=" ", index_col=[0, 1], header=[0, 1]
                        ),
                        pd.read_csv(
                            _qoi_t_part_path, sep=" ", index_col=[0, 1], header=[0, 1]
                        ),
                    )
                else:
                    qoi, qoi_t = (
        ...
        qoi_results = pd.read_csv(
            qoi_results_path, sep=" ", index_col=[0, 1], header=[0, 1]
        )
        qoi_results_t = pd.read_csv(
            qoi_results_t_path, sep=" ", index_col=[0, 1], header=[0, 1]
        )
    ...
    if any("Tcp" in s for s in config_descriptions.keys()):
        ...
        if True:
            plot_pvar(
                ...
                "rcvdSinrUl",
                x_label="Mean HTTP Response Size [kB]",
                y_label="Uplink SINR",  # ylimits=[5.25, 9],
                x_multiplier=10.0,
                reindex=-1,
        ...
        if True:
            plot_pvar(
                ...
                y_label="D2D Delay (application layer) [ms]",
                xlimits=[44.5, 104.5],
                ylimits=[0, 200],
                x_multiplier=10.0,
                y_multiplier=1000.0,
        ...
        if True:
            plot_pvar(
                ...
                x_label="Inter-TX Time [ms]",
                x_multiplier=10.0,
                legend_loc="lower right",  # ylimits=[40, 50],
                y_label="Uplink SINR",
            )
...
# --- generic utility functions - should be moved to crownetutils lateron
# TODO: move to crownetutils and remove duplicate code here and in analysis.py


def get_parameter_to_sims(run: SuqcStudy, module_name: str, parameter_name: str):
    ...
    for sim in simulations:
        ...
        df_param = ScaveTool().read_parameters(
            sim.sql.sca_path, scave_filter=scave_filter
        )
...
def run_parameter_study(
    base_path: str,
    mobility_sim: MobilitySimulatorType,
    config: str,
    ...
    env.copy_data(base_ini_file=ini_file, scenario_files=[])
...
def analyze_parameter_study(
    base_path: str,
    config: str,
    var_parameter: Dict[str, str],
    ...
    suq_run = SuqcStudy(os.path.join(get_results_dir(base_path), config))
    par_to_sims = get_parameter_to_sims(
        suq_run, var_parameter["module"], var_parameter["name"]
    )
    ...
    if _MP_NR_PROCESSES > 1:
        ...
        for param_value in par_to_sims.keys():
            ...
            for sim in par_to_sims[param_value]:
                (new_vector_data, new_scalar_data) = _analyze_singe_simulation(
                    sim, param_value, vectors, scalars
                )
                # TODO: replace by list operations (cheaper in terms of runtime)
    ...
    aggregated_results = dfs_vector.groupby(level=["param"]).agg(
        ["mean", "std", "max", "min"]
    )
    additional_sca_results = dfs_scalar.groupby(level=["param"]).agg(
        ["mean", "std", "max", "min"]
    )
    aggregated_results = aggregated_results.merge(
        additional_sca_results, left_index=True, right_index=True, how="outer"
    )
    ...
    aggregated_results = pd.concat(
        [aggregated_results], keys=[config], names=["config"]
    )
    ...
    if time_avg_df is not None:
        ...
        if "Tcp" in config:
            data_to_plot = time_avg_df.xs("intWithUnit(exponential(10kB))")[
                "rtt", "mean"
            ]
            plt.scatter(data_to_plot.index, data_to_plot)
            data_to_plot = time_avg_df.xs("intWithUnit(exponential(100kB))")[
                "rtt", "mean"
            ]
            plt.scatter(data_to_plot.index, data_to_plot)
        else:
...
def _analyze_singe_simulation(simulation, parameter_value, vectors, scalars):
    print(
        f"   Analyzing simulation for parameter {parameter_value}: {simulation.data_root}"
    )
    try:
    ...
    vec_data = pd.concat(
        {simulation.label: vec_data}, names=["run_label"], copy=False
    )  # run_label as multi-level idx
    sca_data = pd.concat({simulation.label: sca_data}, names=["run_label"], copy=False)
    vec_data = pd.concat(
        {parameter_value: vec_data}, names=["param"], copy=False
    )  # parameter value as multi-level idx
...
def _retrieve_vector_data(sim, vectors):
    ...
    for vector_module in vectors.keys():
        ...
        if new_data.empty:
            ...
            new_data = pd.merge(
                new_data, vec_data, on=["hostId", "time"], how="outer"
            ).sort_index()
    return new_data
    ...
    for scalar_module in scalars.keys():
        for sca_name in scalars[scalar_module]:
            ...
            if new_data.empty:
                ...
                new_data = pd.concat(
                    [new_data, sca_data], ignore_index=True, sort=False
                )
    return new_data
...
def _time_average(dfs, avg_interval=1.0):
    ...
    aggregated_time_series = dfs.groupby(
        ["param", "run_label"], sort=False, observed=True
    )
    ...
    for key, grp in aggregated_time_series:
        ...
        avg = grp.groupby(
            pd.cut(grp["time"], np.arange(0, _max_time, avg_interval))
        ).aggregate(np.nanmean)
        avg.index = np.arange(0, _max_time, avg_interval)[
            : len(np.arange(0, _max_time, avg_interval)) - 1
        ]
...
def plot_pvar(
    ...
    for config in configs.keys():
        ...
        x_values = [
            float(x) * x_multiplier for x in range(1, len(selected_data.index) + 1, 1)
        ]
        ...
        if reindex != 0:
            ...
            y_err_values = y_err_values.reindex(
                index=np.roll(y_err_values.index, reindex)
            )
        if with_errorbar:
            plt.errorbar(
                x_values,
...
def sim_position_plot(
    base_path, simulation_path, config, file_id, remove_xticks=False, wide_format=False
):
...
if __name__ == "__main__":
    ...
    sim_position_plot(
        "./",
        "../results/study/sumoSimple/simulation_runs/outputs/Sample_1_0/sumoSimple_out/",
        "sumoSimple",
        "sample_1_0",
        remove_xticks=True,
    )
    sim_position_plot(
        "./",
        "../results/study/sumoSimple/simulation_runs/outputs/Sample_2_0/sumoSimple_out/",
        "sumoSimple",
        "sample_3_0",
        remove_xticks=True,
    )
    sim_position_plot(
        "./",
        "../results/study/vadereSimple/simulation_runs/outputs/Sample_1_0/vadereSimple_out/",
        "vadereSimple",
        "sample_1_0",
        remove_xticks=True,
    )
    sim_position_plot(
        "./",
        "../results/study/vadereSimple/simulation_runs/outputs/Sample_3_0/vadereSimple_out/",
        "vadereSimple",
        "sample_3_0",
        remove_xticks=True,
    )
    sim_position_plot(
        "./",
        "../results/study/sumoBottleneck/simulation_runs/outputs/Sample_1_0/sumoBottleneck_out/",
        "sumoBottleneck",
        "sample_1_0",
        wide_format=True,
    )
