...
RESULTS = "../results"
STUDY_NAME = "study_join_notification_fix"
PARAMETER = "*.pNode[*].middleware.VaService.timeClusterJoinNotification"
PARAMETER_NAME = "timeClusterJoinNotification"
    ...
    with open(TEMP_FILE, "r") as f:
        ...
        for sli, sl in enumerate([[1, 11, 2], [3, 4, 5, 10]]):
            ...
            for i, type in enumerate(sl):
                ...
                parameters = sorted(by_vam_type[f"{type}"].keys(), key=lambda e: e[:-1])
        ...
        parameters = sorted(by_param.keys(), key=lambda e: e[:-1])
        ...
        ax.errorbar(
            parameters,
            [np.median(by_vam_type[f"3"][p]) for p in parameters],
            [np.std(by_vam_type[f"3"][p]) for p in parameters],
            color="green",
            marker='o',
            capsize=5,
            markersize=5
        ...
        plt.ylabel("Received join notification VAMs\nby vehicle nodes\n+/- std. deviation", size=18)
        plt.xlabel(f"{PARAMETER_NAME} parameter", size=18)
        fig.tight_layout()
        
        fig.savefig(f"fig/{PARAMETER_NAME}_j.png")    
        fig.savefig(f"fig/{PARAMETER_NAME}_j.svg")
        fig.savefig(f"fig/{PARAMETER_NAME}_j.pdf")
        plt.show()
