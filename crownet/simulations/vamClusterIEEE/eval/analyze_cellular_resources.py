...
def visualize():
    with open(TEMP_FILE, "r") as f:
        ...
        print([[d for d in data[p] if d] for p in parameters])

        plt, fig, ax = setup_fig()
        ax.errorbar(
            parameters,
            [np.mean([d for d in data[p] if not math.isnan(d)]) for p in parameters],
            [np.std([d for d in data[p] if not math.isnan(d)]) for p in parameters],
        ...
        ax.axhline(0.6075, color="purple", label="RB usage w/o clustering")
        ax.legend()
