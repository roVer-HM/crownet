...
class ForwardPropagationAnalysis(Analysis):
    def __init__(
        self, sample_base: SampleBase, quantity_of_interest_values: QuantityOfInterest
    ):
    ...
    def get_results(self):
        ...
        for time_key, qoi in self.qoi_values.get_values().groupby(
            ...
            qoi_independent = qoi.to_numpy()[
                self.sample_base.get_index_independent_samples()
            ]
            df_ = self.get_statistical_data_for_time(
                qoi=qoi_independent, time_key=time_key
            )
            df = pd.concat([df, df_])
        return df
