...
class SensitivityAnalysis(Analysis):
    ...
    def _get_sensitivity_indices(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame()
        for time_key, qoi in self.qoi_values.get_values().groupby(
            by=[QuantityOfInterest.TIME_INDEX_NAME], as_index=False
        ):
            if isinstance(time_key, tuple):
                time_key = time_key[0]
            qoi = qoi.droplevel(QuantityOfInterest.TIME_INDEX_NAME)
            df_ = self.get_sensitivity_indices_for_time(
                qoi=qoi.to_numpy(), time_key=time_key
            )
            df = pd.concat([df, df_])
        return df
    ...
    def get_sensitivity_indices_for_time(self, qoi: np.ndarray, time_key="static"):
        ...
        sobol_indices = analyze(
            problem=self.sample_base.get_parameters_dict(), Y=qoi.ravel()
        )
        first_order_index = self._extract_indices_from_dict(
            "FirstOrderSensitivityIndex", sobol_indices
        )
        total_order_index = self._extract_indices_from_dict(
            "TotalOrderSensitivityIndex", sobol_indices
        )
    ...
    def handle_missing_qoi_vals(self, qoi, time_key):
        ...
        if nan_percentage > 0:
            print(
                f"{nan_percentage * 100:.1f}% of the {self.qoi_values.get_name()} are nan for time_key {time_key}."
            )
            if nan_percentage > self.threshold_failed_sims:
                raise ValueError(
                    f"Threshold value = {self.threshold_failed_sims * 100:.1f}%). Please re-run the simulations."
                )
            else:
                mean = np.nanmean(qoi)
    ...
    def _extract_indices_from_dict(self, index_name, sobol_indices):
        ...
        df = pd.DataFrame(
            ...
            index=pd.MultiIndex.from_product(
                [[index_name], self.sample_base.get_parameter_names()]
            ),
        )
    ...
    def _get_second_order_indices_from_dict(self, sobol_indices):
        ...
        for i1, i2 in np.argwhere(np.isnan(sobol_indices[index_type]) == False):
            ...
            second_order_index_CI_low = (
                second_order_index - sobol_indices[f"{index_type}_conf"][i1, i2]
            )
            second_order_index_CI_high = (
                second_order_index + sobol_indices[f"{index_type}_conf"][i1, i2]
            )

            interaction = self.sample_base.get_interaction_name_from_parameter_index(
                i1, i2
            )
            df_ = pd.DataFrame(
                data=[
...
class SurrogateModelBasedSensitivityAnalysis(SensitivityAnalysis):
    ...
    def _get_sensitivity_indices(self) -> pd.DataFrame:
        ...
        for time_key, qoi in self.qoi_values.get_values().groupby(
            ...
            qoi_surrogate = self.model.evaluate(
                x=self.surrogate_model_sample_base.get_samples()
            )

            df_ = self.get_sensitivity_indices_for_time(
                qoi=qoi_surrogate, time_key=time_key
            )
            df = pd.concat([df, df_])
        return df
