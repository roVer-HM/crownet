...
if 'ROVER_MAIN' in os.environ:
    ...
    _default_opp_container_path = os.path.join(os.environ.get('ROVER_MAIN'), 'scripts/omnetpp')
else:
...
class Config:
    """
    Config object to determine the correct way to call the scave_tool.
    """
    def __init__(self, **kwargs):

        self.scave_tool_cmd = kwargs.get('scave_tool_cmd', _default_scavetool_cmd)
        self.use_docker_container = kwargs.get('use_docker_container', _default_use_docker_container)
        self.opp_container_path = kwargs.get('opp_container_path', _default_opp_container_path)
        self.rover_main = kwargs.get('rover_main', _default_rover_main)

    def test_config(self):
        try:
            out = subprocess.check_output([self.opp_container_path, 'exec', self.scave_tool_cmd, '--help'], env=os.environ.copy())
            out_str = out.decode('utf-8').split('\n')[0:2]
            print('using: ' + ' '.join(out_str))
        except subprocess.CalledProcessError as e:
            raise ConfigException(f"Docker container found but scavetool created error: {e.cmd}")
        except Exception as ee:
            raise ConfigException(f"container_path command not found. container_path was: {self.opp_container_path}")
